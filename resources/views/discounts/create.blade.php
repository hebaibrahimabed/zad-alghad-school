...
<form action="{{ route('discounts.store') }}" method="POST">
    ...
    <div class="form-card">
        <div class="field-row">
            <div class="field-wrap">
                ...
                <input type="text" name="name" value="{{ old('name') }}" required placeholder="مثال: خصم الإخوة">
            </div>
            <div class="field-wrap">
                <label>نوع الخصم <span style="color:#e53935;">*</span></label>
                <select name="type" required>
                    <option value="general" {{ old('type')=='general'?'selected':'' }}>عام (يُطبّق تلقائياً حسب شرط، مثل الإخوة)</option>
                    <option value="special" {{ old('type')=='special'?'selected':'' }}>خاص (يُطبّق يدوياً من موظف التسجيل)</option>
                </select>
                <div class="hint">الخصومات "العامة" تُفحص تلقائياً عند تسجيل كل طالب جديد (حالياً: شرط الإخوة بنفس السنة الدراسية).</div>
            </div>
        </div>
        <div class="field-row">
            <div class="field-wrap">
                ...
                <select name="value_type" required>
                    <option value="percentage" {{ old('value_type')=='percentage'?'selected':'' }}>نسبة مئوية %</option>
                    <option value="fixed" {{ old('value_type')=='fixed'?'selected':'' }}>مبلغ ثابت ₪</option>
                </select>
            </div>
            <div class="field-wrap">
    ...
    <button type="submit" class="btn-save"><i class="fas fa-save"></i> حفظ الخصم</button>
